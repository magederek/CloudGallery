...
(function(IIS) {
	...
	IIS.Slider.prototype.addCaptions = function() {
		...
		Array.prototype.forEach.call(this._attributes.slides, function(slide, i) {
			...
			IIS._addClass(loading, 'loading');
			var tempClass = slide.getAttribute('data-src').split("{")[1].split("}")[0];
			IIS._addClass(loading, tempClass);

			var captionContent = '';
			if (slide.getAttribute('title')) {
				captionContent += '<div class="iis-caption-title">' + slide.getAttribute('title') + '</div><br>';
			}
			if (slide.getAttribute('data-text')) {
				captionContent += '<div class="iis-caption-content">' + slide.getAttribute('data-text')  + '</div>';
			}
			...
			if (captionContent) {
				...
				loading.innerHTML = '<div class="ui active centered large inline loader"></div>'
