...
function defaultLoad(FLAG) {
    if (FLAG == true) {
        _("videoContainer").className = "ui six column grid";
    } else {
        _("videoContainer").className = "ui divided items";
    }
    _("addVideo").addEventListener("change", function () {
        var node = _("videoContainer");
...
function createDataBindingVideo(file) {
    var date = GetDate();
    ...
    var cardView =  '<div class="column" id="'+titleName+'"> ' +
                    ...
                    '</div>';
    if (FLAG == true) {
        $("#videoContainer").append(cardView);
    } else {
        $("#videoContainer").append(itemView);
    }
...
function deleteFile(fileName){
    _("videoContainer").removeChild(_(fileName));
}
function loadFolder() {
    $.ajax({
